...
export class LicenseRtuEditorComponent implements OnInit {
  ...
  saveRTU() {
    // TODO -- perform save operation based on
    // current state of Editor rendering
  ...
  addLumWrapperObjects(formData: any) {
    ...
    if (me.rtuLumAgreement
      ...
      const reqPayload = {
        ...
        // TODO - check with Alex - what values to pass??
  ...
  addUIDIfMissing(formData: any) {
    ...
    if (!formData.uid || me.rtuFormInput.isExampleData) {
      type = 'agreement';
      // TODO - restrictions > check with Alex
      // - is same ui as agreement uid??
      formData.uid = this.createUID(type, companyName);
    }

    // TODO - restrictions > check with Alex - how to handle uid for permission
    ...
    // TODO - restrictions > check with Alex - how to handle uid for prohibition
