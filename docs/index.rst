...
The license right to use (RTU) editor provides a micro front end for the management of
  ...
  - subscriber can review the agreement from the suppliers and save into license usage
    manager
  - subscriber can add restrictions on top of the agreement from the supplier such as
    specifying specific users who can use the software.
  - Angular 8 app that is served using a docker image with nginx web server.
...
When you deploy a model using kubernetes the default profile
will use the ELK stack and report back usage of the model. 
