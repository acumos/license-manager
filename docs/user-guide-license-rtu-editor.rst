...
The right to use editor assets a supplier and subscriber create, review and restrict agreements to use models.
...
4. Subscriber can optionally add restrictions on top of the agreement (such which user names are allowed to use software)
...
When you publish a model with a license profile that indicates right to use required. An agreement / right to use agreement 
for the software must be setup using the right to use editor and license usage manager (LUM) service.
...
- Acumos solution ID is the software persistent id or (lum:swPersistentId) use this if you want to allow all versions of a model.
- Acumos revision Id is the software tag id or (lum:swTagId) use this if you want to provide rights for one or more version(s) of the model.
- The Catalog id is the common data service catalog id you can find in the url when filtering on catalog in portal
- The lum:swProductName is the name of the solution (this will match any solution by name)
...
- Please select actions that are allowed if you choose use then all sub actions will be allowed.
...
The subscriber can review the agreement by looking at the JSON or by using the RTU editor 
to review the data contained in the agreement.

If you have lum service available under the same domain as the rtu editor then you can submit the agreement directly from 
the rtu editor.
...
Did the software get registered with LUM during publishing? – ie is there a SwidTag? yes or no? 
...
– Is there a close match such as there was an agreement for a revision id but now I need a new agreement for new revision.

Are there restricted rights that need to be amended?  Was the target restricted by the restrictions vs supplier agreement? 
...
These should be customized for your licensing agreements. The license rtu editor has a shortcut where 
